  ...
  return (
    <div className="relative h-full flex flex-col bg-black overflow-hidden select-none">
      <div className="z-50 bg-[#0F172A]/90 backdrop-blur-xl p-4 border-b border-white/5 shadow-2xl">
        ...
        <select
          value={activeId}
          onChange={(e) => setActiveReserve(e.target.value)}
          className="w-full bg-slate-900 border border-white/10 rounded-xl p-3 text-sm font-bold text-white appearance-none outline-none focus:border-emerald-500"
        >
          {RESERVES.map(r => (
            <option key={r.id} value={r.id}>
              {r.name} {state.customMapTiles[r.id] ? '★' : ''}
            </option>
          ))}
        </select>
        ...
        <div className="flex gap-2 mt-4 overflow-x-auto pb-1 custom-scrollbar">
          {['ALL', ...Object.values(ZoneType)].map(t => (
            <button
              ...
              className={`whitespace-nowrap px-4 py-2 rounded-lg text-[8px] font-bold uppercase border transition-all ${
                filterType === t
      ...
      >
        ...
        >
          {hasTiles ? (
            <div className="grid grid-cols-2 grid-rows-2 w-[1600px] h-[1600px] bg-slate-900 border border-white/10">
              {['nw', 'ne', 'sw', 'se'].map(t => (
                <div
                  key={t}
                  className="w-full h-full border border-white/5 flex items-center justify-center"
                >
                  {tiles[t as keyof MapTiles] ? (
                    <img
                      src={tiles[t as keyof MapTiles]}
            ...
            <img
              src={reserve?.mapUrl}
              className="w-[100vw] h-auto block opacity-40 grayscale"
              style={{ imageRendering: '-webkit-optimize-contrast' } as any}
            />
          )}
          ...
          {state.zones
            .filter(
              (z: Zone) =>
                z.reserveId === activeId &&
                (filterType === 'ALL' || z.type === filterType)
            )
            .map((z: Zone) => (
              <div
                key={z.id}
                className={`absolute -translate-x-1/2 -translate-y-1/2 w-8 h-8 rounded-full border-2 border-white/40 flex items-center justify-center shadow-2xl active:scale-150 transition-transform ${PIN_COLORS[z.type]}`}
                style={{ left: `${z.x}%`, top: `${z.y}%` }}
                onClick={(e) => {
                  e.stopPropagation();
                  if (confirm('Delete?')) deleteZone(z.id);
                }}
              >
      ...
      <div className="absolute bottom-6 right-6 flex flex-col gap-3 z-50">
        <button
          onClick={() => setScale(p => Math.min(10, p + 1.5))}
          className="w-14 h-14 bg-slate-900/90 rounded-2xl border border-white/10 text-white font-bold shadow-2xl"
        >
          ＋
        </button>
        <button
          ...
          className="w-14 h-14 bg-emerald-600/90 rounded-2xl border border-white/20 text-white text-[10px] font-bold shadow-2xl"
        >
          CENTER
        </button>
        <button
          onClick={() => setScale(p => Math.max(0.1, p - 1.5))}
          className="w-14 h-14 bg-slate-900/90 rounded-2xl border border-white/10 text-white font-bold shadow-2xl"
        >
          －
        </button>
